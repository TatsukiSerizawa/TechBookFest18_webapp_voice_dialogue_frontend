  ...
  const startRecording = async () => {
    ...
    mediaRecorder.onstop = async () => {
      try {
        ...
        if (!data.audio_url) throw new Error("audio_url is \
            missing in response");
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col \
      ...
      <div className="mt-6 flex items-start space-x-4">
        ...
        <div className="bg-white shadow-md rounded-lg p-4 max-w-lg relative">
          ...
          <div className="absolute top-6 left-[-10px] w-0 h-0 border-t-8 \
            border-b-8 border-r-8 border-transparent border-r-white"></div>
        </div>
      </div>
      <button
        onClick={recording ? stopRecording : startRecording}
        className={`px-6 py-3 rounded-lg text-white font-semibold transition ${ recording ? "bg-red-500 hover:bg-red-600" : "bg-blue-500 hover:bg-blue-600"
        }`}
      >
